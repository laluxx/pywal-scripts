...
def write_xmobar_config(base_config, colors):
    ...
    # Replace bgColor with color0
    config = re.sub(r'bgColor\s*=\s*".*?"', f'bgColor      = "{colors["color0"]}"', config, count=1)
    ...
    # Replace other color instances with a color from the cycle
    config = re.sub(r'(#)[#\da-fA-F]+', lambda match: next(color_cycle) if match.group() != colors["color0"] else match.group(), config)
